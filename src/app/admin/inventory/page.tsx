...
import { Boxes, Plus, Minus, Edit3, History, AlertTriangle, CheckCircle, Search, X } from 'lucide-react';
import { initialProducts, initialInventoryHistory } from '@/lib/mock-data';
import { formatDateTime } from '@/lib/utils';
import { Product, InventoryHistoryItem } from '@/types/database';

export default function AdminInventoryPage() {
  const [products, setProducts] = useState<Product[]>(initialProducts);
  const [history, setHistory] = useState<InventoryHistoryItem[]>(initialInventoryHistory);
  ...
  const handleApplyAdjustment = (e: React.FormEvent) => {
    ...
    const prevQty = selectedProduct.stock_quantity;
    let newQty = prevQty;
    let qtyChanged = 0;
    ...
    setProducts((prev) =>
      prev.map((p) => (p.id === selectedProduct.id ? { ...p, stock_quantity: newQty } : p))
    );

    // Append to audit history
    const auditRecord: InventoryHistoryItem = {
    ...
    setHistory((prev) => [auditRecord, ...prev]);
            ...
            <tbody className="divide-y divide-street-800/60">
              {history.map((h) => (
                <tr key={h.id} className="hover:bg-street-900/30">
                  ...
                  <td className="py-2.5 text-zinc-400">{h.admin_user}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="bg-card border border-street-800 rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleApplyAdjustment} className="space-y-4 text-xs">
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-street-800">
                ...
                <button
                  type="submit"
                  className="bg-brand-neon text-black font-black uppercase px-5 py-2 rounded-xl shadow-glow-neon"
                >
                  Commit Stock Update
                </button>
              </div>
